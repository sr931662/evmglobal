...
export default function PackageCard({ pkg, index }) {
  ...
  // Derived, so a package saved without a nights value still shows one where
  // the itinerary or hotels say how long the trip is.
      ...
      <div className={styles.imgWrap} onClick={() => navigate(`/package-details/${pkg.slug || pkg.id}`)}>
        ...
        {nights > 0 && (
          <div className={styles.nightsBadge}>
            <span className={styles.nightsStar}>◑</span> {nights} Nights
          </div>
        )}
