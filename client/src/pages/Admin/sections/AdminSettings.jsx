    ...
    <div className="space-y-8 max-w-3xl">
      ...
      <SettingsSection title="Business Profile" delay={0.05}>
        <div className="space-y-5">
          {[
            { label: 'Business Name',  value: 'EaseMyVacations Global',  placeholder: '' },
            { label: 'Email Address',  value: 'concierge@emvglobal.in',  placeholder: '' },
            { label: 'Phone / WhatsApp', value: '+91 98765 43210',        placeholder: '' },
            { label: 'Website',        value: 'www.emvglobal.in',         placeholder: '' },
            { label: 'Registered Address', value: 'Mumbai, Maharashtra, India', placeholder: '' },
          ].map(f => (
      ...
      {/* Team Members */}
      <SettingsSection title="Team Members" delay={0.1}>
        <div className="space-y-4 mb-6">
          {[
            { name: 'Shivam Rastogi',  role: 'Admin',     avatar: 'S', color: 'bg-brand' },
            { name: 'Priya Sharma',    role: 'Concierge', avatar: 'P', color: 'bg-blue-500' },
            { name: 'Rahul Verma',     role: 'Sales',     avatar: 'R', color: 'bg-purple-500' },
          ].map(m => (
            <div key={m.name} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-2xl ${m.color} text-white font-bold flex items-center justify-center text-lg`}>{m.avatar}</div>
                <div>
                  <p className="font-bold text-dark">{m.name}</p>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{m.role}</p>
                </div>
              </div>
              <button className="text-sm font-bold text-gray-400 hover:text-red-500 transition-colors px-3 py-1.5 rounded-xl hover:bg-red-50">Remove</button>
            </div>
          ))}
        </div>
        <button className="text-sm font-bold text-brand border border-brand/20 bg-brand/5 px-5 py-2.5 rounded-full hover:bg-brand/10 transition-colors">
          + Invite Team Member
        </button>
      </SettingsSection>
      ...
      <SettingsSection title="WhatsApp Integration" delay={0.2}>
        <div className="space-y-5">
          <div className="flex items-center gap-4 p-5 bg-[#25D366]/5 rounded-2xl border border-[#25D366]/20">
            ...
            <div>
              <p className="font-bold text-dark text-sm">WhatsApp Business Connected</p>
              <p className="text-[#25D366] text-sm font-bold">+91 98765 43210 · Active</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
            ...
            <input defaultValue="+919876543210" className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3 text-dark font-bold text-sm focus:outline-none focus:border-brand transition-colors" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.25em]">Default Message Template</label>
            <textarea
              defaultValue="Hi! I'd like to enquire about {packageName}. Please share details and availability."
              rows={3}
      ...
      <div className="flex justify-end gap-4 pb-4">
        <button className="px-8 py-3.5 rounded-full border border-gray-200 text-gray-600 font-bold hover:bg-gray-50 transition-colors text-sm">Reset</button>
        <button
          onClick={handleSave}
          className={`px-10 py-3.5 rounded-full font-bold text-sm transition-all ${saved ? 'bg-green-500 text-white' : 'bg-dark text-white hover:bg-black shadow-sm'}`}
        >
